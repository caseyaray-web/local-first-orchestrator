...
A bounded `process-next` controller operation now exists. It serializes ticks with a restart-recoverable lease, atomically claims dependency-ready admission and implementation work, persists claim/reclaim/started/completed/finalized evidence, honors pause, exposes active scheduler state, and executes at most one generated-card, state-projection, evidence-comment, implementation, or dependency-readiness stage per invocation. The implementation stage now persists model-launch intent before inference, refuses to reinvoke unknown or terminal-failed invocations, recovers a completed invocation whose model-stage record was interrupted, and replays a persisted model stage without re-inference. The registered CLI keeps dry-run as the default and requires both execution and board-write gates for live processing. Repository tests cover dependency exclusion, malformed legacy dependency data, overlapping ticks, expired-lease restart, stage-by-stage outbox delivery, implementation preview, and the model-stage crash boundaries. It remains partial because validation/review, integration/completion, native dependency release, and paid-call stages are not yet routed through this scheduler, and the full crash matrix across those remaining effects has not been proven.
