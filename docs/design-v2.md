...
A bounded `process-next` controller operation now exists. It serializes ticks with a restart-recoverable lease, atomically claims dependency-ready admission work, persists claim/reclaim/completion evidence, honors pause, exposes active scheduler state, and executes at most one existing generated-card, state-projection, evidence-comment, or dependency-readiness stage per invocation. The registered CLI keeps dry-run as the default and requires both execution and board-write gates for live processing. Repository tests cover dependency exclusion, malformed legacy dependency data, overlapping ticks, expired-lease restart, and stage-by-stage outbox delivery. It remains partial because model/validation/review, integration/completion, native dependency release, and paid-call stages are not yet routed through this scheduler, and the full crash matrix across those effects has not been proven.
