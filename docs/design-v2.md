...
### 1. Restartable `process-next` scheduler — **Partial**

A bounded `process-next` controller operation now exists. It serializes ticks with a restart-recoverable lease and routes dependency-readiness, implementation, deterministic validation, and fresh review as independent durable stages alongside generated-card, state-projection, and evidence-comment work. Implementation persists model-launch intent before inference and recovers completed work without duplicate invocation; validation binds exact candidate/diff/worktree identity; review is packet-only, binds the validated candidate plus configured provider/model/timeout/schema identity, preserves adapter-authored artifacts, recovers completed invocations interrupted before model-stage persistence, and refuses ambiguous started invocations. The registered CLI keeps dry-run as the default and requires both execution and board-write gates for live processing. Repository tests cover dependency exclusion, malformed legacy dependency data, overlapping ticks, expired-lease restart, stage-by-stage outbox delivery, implementation/validation/review preview and recovery, candidate drift, execution-policy drift, and model-stage crash boundaries. It remains partial because repair routing, triage scheduling, acceptance/integration/completion, native dependency release, tranche/checkpoint, and paid-call stages are not yet routed through this scheduler, and the scheduler-wide crash/concurrency matrix has not been proven.
...
### 2. Daemon / continuous orchestration loop — **Not started**

There is no complete unattended daemon that continuously polls, claims, processes, pauses, resumes, and recovers work according to the design. Existing commands are oriented around bounded or explicit operator-triggered runs.
...
### 3. Full low-risk end-to-end acceptance path — **Not started**

The repository has unit tests and fake-adapter vertical slices, but it has not yet demonstrated the design's strongest acceptance case using real configured integrations: architecture/decomposition through implementation, deterministic validation, fresh review, commit, board completion, and zero unnecessary paid post-architecture calls.
...
### 4. Hermes-native dependency graph projection and release — **Partial**

Version 2.0 explicitly requires Local First's dependency DAG to be projected into Hermes-native task links before release so Hermes can own ordinary readiness/execution semantics. The design identifies this as an intended fix, and the repository has Hermes adapters and projection machinery, but exact native dependency graph equivalence and release behavior are not yet proven complete.

**Completion condition:** projected active-tranche cards receive exact Hermes parent/dependency links, graph equivalence is verified, and release leaves readiness determination to Hermes as designed.
...
### 6. Live Hermes read/write integration proof — **Partial**

The repository contains `HermesBoardAdapter`, explicit board-write gates, board projection logic, and fake-adapter tests. What remains is live operational proof against the actual Hermes board APIs and current native task/dependency semantics.
...
### 7. Real local implementation-model execution — **Partial**

The local Qwen adapter and isolated worktree execution path exist. The repository is wired for local Hermes chat/model execution, but the design-level proof requires a real configured model completing representative implementation work under the constrained context/tool policy.
...
### 8. Real fresh review-model execution — **Partial**

The review worker, exact schema, criterion-level verdicts, and pass/repair/escalate contract exist. The missing portion is live proof that review runs independently from implementation and produces accepted structured evidence under the real runtime.
...
### 10. Evidence-comment reconciliation — **Partial**

Evidence-comment delivery has durable/outbox-oriented machinery, but the currently audited Hermes read surface does not expose enough comment-body data to prove marker-based exactly-once reconciliation through the live CLI/API path.
...
### 11. Paid checkpoint/model operational integration — **Partial**

The repository contains usage-governor concepts, reservations, budgets, purpose binding, and paid-adapter abstractions. The remaining work is production integration and real approval/exhaustion behavior.

**Completion condition:** every paid invocation is atomically reserved before execution, purpose-scoped, auditable, budget-limited, and proven to pause rather than overspend when the budget is exhausted.
...
| Hermes-native v2 execution/projection additions | **Partial, with bounded scheduler foundations present and daemon still not started** |
...
The main unfinished work is now concentrated in **orchestration and operational proof**: extend the bounded restartable scheduler across the remaining trust lifecycle, build the daemon, complete Hermes-native graph projection/execution reconciliation, then demonstrate the whole design against real Hermes and real configured model processes under crash/restart conditions.

Until those items are demonstrated, the system should be considered **substantially implemented but not yet fully operational or acceptance-complete**.
