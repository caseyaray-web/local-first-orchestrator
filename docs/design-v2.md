...
### 4. Hermes-native dependency graph projection and release — **Complete for representative multi-ticket live path**

The scheduler resolves authoritative Local First↔Hermes task identities, projects missing native parent links with Hermes `link`, verifies the exact native parent set, records immutable graph evidence, and keeps dependent generated tickets locally `draft` until Hermes itself reports them `ready`. Handoff-marked dependent cards are unblocked only after graph convergence, so Hermes owns `todo -> ready` while parents remain open/completed. Native release then atomically records immutable release evidence and moves the local child `draft -> ready_local`; because Hermes readiness was just observed authoritatively, the matching ready-state projection is acknowledged locally rather than sent back as a redundant remote unblock. Graph divergence and provenance drift stop reconciliation.

The real multi-ticket proof projected `T1-A -> T1-B`, observed A `ready` and B `todo` with exact parent A, let Hermes advance B to `ready` only after A completed, and persisted the exact graph/release hash before B execution. B's dispatcher worktree was pre-materialized at A's accepted integration commit, proving dependency release and execution-base lineage together. Detailed evidence is in `docs/multi-ticket-native-tranche-acceptance.md`.

**Completion condition:** projected active-tranche cards receive exact Hermes parent/dependency links, graph equivalence is verified, release leaves readiness determination to Hermes as designed, and the behavior is demonstrated against the live integration. **Satisfied for the representative multi-ticket live path.**

### 5. Hermes execution reconciliation — **Complete for representative dispatcher-owned execution**

The dispatcher-owned execution path is implemented and live-proven for a real Local First-generated Hermes task. Generated/triage cards carry a Local First execution-handoff marker and are created as real Hermes Git worktrees. `ready_local` and `repairing` release to Hermes through native `unblock`; generated cards are permanently excluded from the Local First implementation claimant. The scheduler polls acknowledged generated tickets for Hermes run state before the local implementation slot, automatically reconciles only unreconciled blocked handoff runs carrying the exact `local-first-awaiting-reconciliation` sentinel, and supports subsequent Hermes-owned repair attempts without falling back to Local First implementation.

A reconciled dispatcher run is bound to the exact Local First ticket through acknowledged generated-card provenance, its worktree is verified against the configured repository/common Git directory and authoritative tranche execution base, and one atomic ledger transaction creates/reuses the attempt plus immutable `hermes_execution_reconciliations` and `model_stage_artifacts(adapter='hermes-dispatch')` evidence. Deterministic validation and review are source-aware: Hermes execution provenance replaces a Local First implementation model invocation, while the same base/diff identity checks remain authoritative. Git integration accepts an out-of-root Hermes worktree only when immutable reconciliation evidence matches the exact ticket/attempt/path/base/fingerprint; tranche checkpoint can reconstruct a temporary detached worktree from the durable final integration commit after Hermes removes its worker workspace.

The live proof used Hermes task `t_6ddb14ba`. Worker run `1` ended `blocked/blocked` with the exact reconciliation sentinel and remained non-final through Local First validation, independent review, acceptance, and Git integration. Local First launched **zero implementation model invocations** and exactly one independent review invocation. Accepted commit `59b1b6c0da7bc7402d4c12758ccf2b268e0e5688` was integrated, Local First reached `done`, and only Local First then projected Hermes `done`. A later Hermes run with summary `local-first projection ticket-event:99` was the final Local First completion projection, not a second implementation worker. Detailed evidence is in `docs/hermes-dispatch-execution-acceptance.md`.

**Completion condition:** a Hermes-dispatched worker run is detected, bound to the correct Local First attempt, reconciled into the ledger, and forced through deterministic validation/review before graph advancement. **Satisfied for the representative dispatcher-owned path.**

### 6. Live Hermes read/write integration proof — **Partial, substantially proven by Milestone 20**

Milestone 20 exercised the real Hermes CLI/API surface for task reads, scheduled ownership, state projection, comments, marker lookup, terminal completion, native dependency links, and retry/reconciliation after an ambiguous comment-delivery crash. The run exposed and fixed stale adapter assumptions around comment-body availability, already-scheduled idempotence, and scheduled completion. The complete multi-ticket scheduler-generated tranche/card graph with native dependency release and dispatcher-owned reconciliation on each ticket is now live-proven. Remaining live proof for this broader item is concentrated in exhaustive external-boundary crash/restart behavior and the live paid-provider path.
...
### 11. Paid checkpoint/model operational integration — **Complete for representative live provider route**

The scheduler routes deterministic `ready_for_checkpoint` evidence into purpose-scoped paid checkpoint and escalation stages using the existing usage governor. Claim identity binds feature/tranche/checkpoint lineage plus provider/model/profile provenance, the deterministic scheduler claim ID is the governor request key, and the production `HermesPaidModelAdapter` invokes packet-only safe Hermes chat with explicit provider/model selectors. Durable reservations precede provider execution, completed responses are bound to immutable paid evidence, budget exhaustion performs no provider call until an explicit one-call `approve-paid` authorization is recorded, and ambiguous outcomes are marked `unknown_outcome` and never reinvoked for that request. A live acceptance run against the configured `openai-codex` / `gpt-5.6-sol` route proved zero-budget pause, one-call approval, exactly one real checkpoint invocation, crash-before-scheduler-apply completed-response replay without duplicate spend, purpose-scoped checkpoint→escalation chaining to one real escalation call, and a real-provider ambiguity boundary where the provider completed but the result was deliberately dropped: restart made zero additional provider calls and preserved durable `unknown_outcome`. Detailed evidence is in `docs/live-paid-provider-acceptance.md`.
...
The live multi-ticket proof completed two dispatcher-owned tickets in one generated tranche, advanced the rolling integration head in dependency order, recorded immutable tranche completion evidence, ran deterministic tranche integration verification, and persisted `ready_for_checkpoint`. A purpose-scoped deterministic approval through the existing paid-governor/scheduler path then authorized successor materialization without claiming live paid-provider acceptance. The registered real standard planner re-snapshotted against the predecessor final integration head, materialized only the coarse successor criterion, projected the successor card, and `next_tranche_activation` froze exact ticket/external-task/graph evidence. A separate clean successor activation verification proved the final runtime binding and prepared Hermes worktree use the predecessor final integration SHA. Detailed evidence is in `docs/multi-ticket-native-tranche-acceptance.md`.

**Completion condition:** a multi-ticket tranche completes, integrates, validates against the expected repository state, checkpoints correctly, and activates the next tranche without bypassing trust policy. **Satisfied for the representative multi-ticket live path.**

### 15. Same-ticket repair machinery — **Complete for deliberate deterministic failure path**

Bounded repair logic, failure fingerprints, same-ticket retry continuity, retry limits, and repeated-failure routing are implemented and acceptance-proven. A deliberate deterministic validation failure now exercises the full scheduler path twice in a real temporary Git/worktree/controller environment: attempt 2 remains the same ticket and reuses the same worktree/branch, compact prior failure evidence is injected into the repair packet, the repeated stable fingerprint routes exactly once to `needs_triage`, restart drains projection before triage, and exactly one triage invocation/child projection is materialized. Detailed evidence is in `docs/failure-driven-repair-triage-acceptance.md`.
...
| Phase 3 — Local review and same-ticket repair | **Complete for representative live review and deliberate deterministic failure-driven repair** |
| Phase 4 — Bounded triage | **Complete for bounded policy plus repeated-failure → restart → exactly-once triage proof** |
| Phase 5 — Architecture/checkpoint and paid governor | **Complete for representative tranche/checkpoint and live paid-provider checkpoint/escalation route** |
| Phase 6 — Symbol-aware context and metrics | **Partial; bounded context complete, production feedback loop pending** |
| Hermes-native v2 execution/projection additions | **Complete for representative configured paths; scheduler/daemon/live acceptance, dispatcher-owned execution reconciliation, multi-ticket native graph, live paid-provider operation, and representative external-boundary crash/restart proof are complete** |
...
The dependency-ordered scheduler, daemon, representative real low-risk Local First-owned acceptance path, dispatcher-owned execution reconciliation, the **full multi-ticket scheduler-generated native tranche graph proof**, deliberate failure-driven repair/triage path, representative live paid-provider checkpoint/escalation route, and representative external-boundary crash/restart matrix are now complete. The crash acceptance covers a real Hermes ambiguous write, real configured local implementation ambiguity and durable-result replay, real fresh-review replay, actual Git commit/ref recovery, real checkpoint artifact/integration-command replay, and real paid-provider completed/unknown-outcome behavior. Detailed evidence is in `docs/multi-ticket-native-tranche-acceptance.md`, `docs/failure-driven-repair-triage-acceptance.md`, `docs/live-paid-provider-acceptance.md`, and `docs/live-external-boundary-crash-acceptance.md`. The remaining full-v2 work is operator lifecycle/recovery UX followed by runtime metrics/adaptive sizing.

The scheduler roadmap is **acceptance-complete for the representative low-risk Local First-owned path** demonstrated in Milestone 20; dispatcher-owned Hermes execution reconciliation is acceptance-complete for the representative generated-ticket path; the multi-ticket native tranche graph/successor-activation path is acceptance-complete; paid checkpoint/escalation operation is acceptance-complete for the representative configured live provider route; and representative external-effect crash/restart behavior is acceptance-complete across Hermes/model/review/Git/checkpoint/paid effect classes. The broader v2 design remains operationally incomplete only for operator/product UX and runtime metrics/adaptive sizing.
