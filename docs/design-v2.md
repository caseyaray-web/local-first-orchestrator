...
### 5. Hermes execution reconciliation — **Partial (explicit durable handoff implemented; automatic detection/finality control pending)**

The first production reconciliation slice is implemented. `HermesBoardAdapter.execution_snapshot()` reads one atomic Hermes task/run/graph snapshot. `reconcile-hermes-execution` binds an explicitly selected completed non-Local-First Hermes run to the exact Local First ticket by external task id, verifies that the reported workspace belongs to the configured Git repository, requires the worker `HEAD` to descend from the authoritative execution base, fingerprints the exact `git diff <base>` candidate, and atomically creates one Local First attempt plus one `model_stage_artifacts(stage='implementation', adapter='hermes-dispatch')` row and immutable `hermes_execution_reconciliations` evidence. Replay returns the same attempt; immutable conflicts fail closed; no Local First implementation model invocation is launched; and the existing validation scheduler immediately sees the reconciled candidate. A Hermes task already marked final `done` is rejected with `hermes_completion_authority_bypassed_reconciliation_required`, because native dependents may already have been released.

What remains is the automatic ownership path: Local First must detect/claim eligible dispatcher-owned run completions before its own implementation scheduler can launch a duplicate worker, and the Hermes lifecycle must keep worker completion non-final until Local First deterministic validation/review/integration authorizes final `done`. After that, the explicit handoff must be demonstrated against a real dispatcher-owned worker run.
...
The dependency-ordered scheduler, daemon, and representative real low-risk Local First-owned acceptance path are now complete. The main unfinished work is concentrated in broader operational integration. **Hermes execution reconciliation is now in progress:** the explicit replay-safe Hermes-run → Local First attempt/provenance handoff is implemented and validation-compatible; the next dependency is automatic dispatcher-run detection/ownership plus a Hermes lifecycle hook that prevents worker completion from becoming final `done` before Local First validation/review/integration authority. After that, the remaining major proof sequence is a full scheduler-generated native tranche graph exercise, exhaustive live external-boundary crash injection, live paid-provider checkpoint/escalation operation, then the remaining operator/metrics/productization work.
