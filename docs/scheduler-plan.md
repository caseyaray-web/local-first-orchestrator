...
2. **Full scheduler-generated native tranche graph proof — Complete.** A real Local First-generated T1 chain (`T1-A -> T1-B`) was projected to Hermes, Hermes owned native `todo -> ready` readiness, both tickets executed through real dispatcher workers with zero Local First implementation calls, the second worktree was pre-anchored to the first accepted integration commit, T1 integrated/checkpointed, a deterministic approved checkpoint unlocked the real standard successor planner, and TR2 materialization/projection/activation completed against the final T1 integration SHA. A separate clean successor activation verification proved the final runtime binding/worktree base fix. Detailed evidence: `docs/multi-ticket-native-tranche-acceptance.md`.
3. **Representative live external-boundary crash proof — Complete.** Beyond the deterministic matrix and production-adapter Hermes state/comment/dependency ambiguity tests, the configured system now has representative real crash/restart evidence for a live Hermes remote write, ambiguous and completed-result local implementation invocations, fresh-review replay, actual Git commit/ref mutation, checkpoint artifact/integration-command replay, and paid-provider completed/unknown-outcome behavior. Evidence: `docs/crash-matrix-hardening.md`, `docs/live-external-boundary-crash-acceptance.md`, and `docs/live-paid-provider-acceptance.md`.
4. **Live paid checkpoint/escalation integration — Complete for representative configured route.** A zero-budget checkpoint made no provider call, one explicit approval authorized exactly one real checkpoint call, completed-call replay reused the durable response after a simulated crash, checkpoint→escalation chaining executed one real escalation call, and a deliberately lost real-provider response became durable `unknown_outcome` with zero restart calls. Evidence: `docs/live-paid-provider-acceptance.md`.
5. **Operator lifecycle/recovery UX — Complete.** `init`, bounded `operator-status`, durable/audited `pause`/`resume`, `doctor`/`recovery-status`, the existing stage-specific recovery/reconciliation commands, one-call paid approval, and explicit daemon/process-next diagnostics now form one documented operator workflow. Evidence: `docs/operator-lifecycle-recovery.md`.
6. **Runtime metrics and adaptive sizing — Complete.** Completed-ticket observations are append-only/idempotent, operator metrics include runtime/outcome and paid token/call telemetry, and a minimum-sample bounded policy adjusts only decomposition active-ticket/context hints. Initial and successor planning persist the chosen recommendation in provenance. Evidence: `docs/runtime-metrics-adaptive-sizing-acceptance.md`.
