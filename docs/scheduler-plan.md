...
3. **Crash-matrix hardening — Complete for production adapters; broader live-service breadth remains.** Production adapter ambiguity proofs now cover Hermes state/comment/dependency effects, and the real paid-provider boundary proves completed-call replay plus unknown-outcome no-repeat. Remaining work is controlled termination at the other configured live model/review/Git/checkpoint boundaries where operationally safe. Evidence: `docs/crash-matrix-hardening.md` and `docs/live-paid-provider-acceptance.md`.
