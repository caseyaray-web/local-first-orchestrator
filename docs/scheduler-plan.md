...
3. **Exhaustive live external-boundary crash proof — next.** Extend the scheduler-wide crash matrix from deterministic/fake-runtime proof plus the existing live comment/Git/checkpoint recovery evidence to real model, review, Git, board, dependency, checkpoint, and paid boundaries.
