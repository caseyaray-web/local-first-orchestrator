...
class FailedAttemptReconciliationTests(unittest.TestCase):
    ...
    def test_symbol_scoped_historical_failure_is_rejected_by_governed_revalidation(self) -> None:
        import local_first_orchestrator.validation as validation_module
        old = lambda *args: (("symbol scope exceeded in test file: app.py",), False)
        ...
        self.ledger.pause("operator", reason="classifier gate")
        with self.assertRaisesRegex(ValueError, "recognized obsolete controller defect"):
            self.controller(Model("good")).revalidate_historical_implementation(self.ticket, 1, repository=self.repo)

    def test_revalidation_fails_closed_before_mutation_checks_for_unrecognized_scope_failure(self) -> None:
        import local_first_orchestrator.validation as validation_module
        old = lambda *args: (("symbol scope exceeded in test file: app.py",), False)
        with mock.patch.object(validation_module, "enforce_symbol_scope", old):
            self.assertIsNone(self.controller(Model("good")).execute_implementation(self.ticket, repository=self.repo))
    ...
    def test_revalidation_current_failure_stays_triage_and_rejects_unrecognized_history(self) -> None:
        import local_first_orchestrator.validation as validation_module
        old = lambda *args: (("verification command failed: npm test",), False)
        with mock.patch.object(validation_module, "enforce_symbol_scope", old):
            self.assertIsNone(self.controller(Model("good")).execute_implementation(self.ticket, repository=self.repo))
        self.ledger.pause("operator", reason="classifier gate")
        with self.assertRaisesRegex(ValueError, "recognized obsolete controller defect"):
