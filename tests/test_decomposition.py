...
class Plans(unittest.TestCase):
 ...
 def test_activation_persists_active_generated_tickets_and_create_projections(self):
  d=TemporaryDirectory(); l=Ledger(Path(d.name)/'x.db');l.migrate(); f=self.feature();p=self.plan();v=PlanValidator().validate(f,p); ok=type('R',(),{'passed':True})(); ticket_ids=activate_validated_plan(l,f,p,v,ok)[1]
 ...
 def test_activation_rolls_back_ticket_and_projection_pair(self):
  ...
  l=Ledger(Path(d.name)/'x.db', failure_injector=fail);l.migrate(); f=self.feature();p=self.plan();v=PlanValidator().validate(f,p); ok=type('R',(),{'passed':True})();
