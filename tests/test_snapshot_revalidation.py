...
class SnapshotRevalidationTests(unittest.TestCase):
    ...
    def test_chain_and_exact_replay_are_guarded(self):
        ...
        replay = self.ledger.append_feature_snapshot_revalidation(feature_id="F", feature_contract_hash=self.feature.contract_hash, repository_identity=str(self.repo.resolve()), repo_base_sha=self.base, source_snapshot_hash=self.s1.snapshot_hash, target_snapshot_hash=s2.snapshot_hash, generation=1, revalidation_hash=h1)
        self.assertEqual(dict(replay), dict(first)); self.assertEqual(self.ledger.connection.execute("select count(*) from feature_repository_snapshot_revalidations").fetchone()[0], 1)
        ...
        historical = self.ledger.append_feature_snapshot_revalidation(feature_id="F", feature_contract_hash=self.feature.contract_hash, repository_identity=str(self.repo.resolve()), repo_base_sha=self.base, source_snapshot_hash=self.s1.snapshot_hash, target_snapshot_hash=s2.snapshot_hash, generation=1, revalidation_hash=h1)
        self.assertEqual(historical["generation"], 1); self.assertEqual(self.ledger.feature_snapshot_authority("F")["snapshot_hash"], s2.snapshot_hash)
        with self.assertRaisesRegex(ValueError, "conflicting snapshot revalidation"):
