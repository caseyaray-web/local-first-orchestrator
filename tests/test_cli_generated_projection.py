...
class GeneratedProjectionCliTests(unittest.TestCase):
 def test_project_generated_requires_existing_write_gate_before_claim(self):
  ...
  plans=Plans();feature=plans.feature();plan=plans.plan();create_and_activate_validated_plan(l,feature,plan,PlanValidator().validate(feature,plan),plans.repository_validation(plan)); l.close()
