...
class Planner(unittest.TestCase):
 def test_fake_process_parses_packet_and_persists_artifacts(self):
  ...
  d=TemporaryDirectory();got=LocalDecompositionPlanner(run,cost_class='local').propose(f,s,artifact_dir=Path(d.name));self.assertEqual(got.feature_id,'F');self.assertIn('do not invent criteria',packet(f,s));self.assertTrue((Path(d.name)/'planner-response.json').exists());d.cleanup()
