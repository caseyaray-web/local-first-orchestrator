...
class ProcessReadTests(unittest.TestCase):
 ...
 def test_status_projection_uses_process_and_explicit_board(self):
  a=HermesBoardAdapter(executable=str(self.exe),board='board',allow_writes=True); a.set_state('1',CanonicalState.READY_LOCAL,idempotency_key='key'); self.assertIn('kanban --board board schedule 1 local-first projection ready_local (key)',self.log.read_text())
