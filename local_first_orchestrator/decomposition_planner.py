...
def planner_contract(*, budget_policy=PATCH_BUDGET_POLICY) -> dict:
    return {
        ...
        'output_rules': [
            'planning only: return exactly one object matching output_contract.schema',
            ...
            'allowed_files are existing contract-authorized modify blobs, including opaque documentation/configuration blobs; create_files are absent contract-authorized non-test create paths; new_test_files are absent contract-authorized test creations',
            ...
            'declared paths are allowed_files plus create_files plus new_test_files and must be unique; primary_symbol is an existing selected-evidence symbol anchor and never a future create symbol; opaque modify blobs must use a related source anchor',
