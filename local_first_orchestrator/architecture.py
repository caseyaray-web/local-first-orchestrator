    ...
    with ledger._transaction() as conn:
        ...
        for ordinal, (tranche, tickets) in enumerate(parsed):
            ...
            for item in tickets:
                ...
                conn.execute("INSERT INTO tickets(id, external_id, feature_id, tranche_id, title, objective, criterion_ids_json, primary_symbol, allowed_files_json, new_test_files_json, forbidden_changes_json, patch_budget_json, verification_json, risk, review_required, max_attempts, dependencies_json, state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (item.ticket_id, item.ticket_id, feature_id, tid, item.ticket_id, contract["objective"], json.dumps(contract["criterion_ids"]), contract["primary_symbol"], json.dumps(contract["allowed_files"]), json.dumps(contract.get("new_test_files", [])), json.dumps(contract["forbidden_changes"]), json.dumps(contract["patch_budget"]), json.dumps(contract["verification"]), contract["risk"], int(item.review_required), item.max_attempts, json.dumps(contract["dependencies"]), state, now, now))
